...
def update(
    ...
    if not version:
        raise HTTPException(
            status_code=400,
            detail="Keine Version angegeben.",
        )
    ...
    try:
        ...
        if not health_ok:
            raise RuntimeError(
                "App-Healthcheck nach dem Update fehlgeschlagen."
            )
        ...
        # --------------------------------------------------------
        # 8. Tatsächliche App-Version prüfen
        # --------------------------------------------------------
        ...
        if current_version != prepared["version"].lstrip("v"):
            raise RuntimeError(
                f"Versionsprüfung fehlgeschlagen. "
                f"Erwartet: {prepared['version'].lstrip('v')}, "
                f"tatsächlich: {current_version}"
            )
        ...
        return {
            "status": "accepted",
            "message": "Release vorbereitet und Backup erstellt.",
        ...
        install_status["phase"] = "rollback"

        try:
            # ----------------------------------------------------
            # Projektdateien zurücksetzen
            # ----------------------------------------------------

            if install_status.get("backup"):
                log(
                    "Stelle Projektdateien aus dem Backup wieder her.",
                    "rollback",
                )

                restore_backup(
                    install_status["backup"]
                )

                log(
                    "Projektdateien wurden wiederhergestellt.",
                    "rollback",
                )

            # ----------------------------------------------------
            # Docker-Image zurücksetzen
            # ----------------------------------------------------

            if install_status.get("rollback_image"):
                log(
                    "Stelle vorheriges Docker-Image wieder her.",
                    "rollback",
                )

                restore_image(
                    install_status["rollback_image"]
                )

                log(
                    "Docker-Image wurde wiederhergestellt.",
                    "rollback",
                )

            # ----------------------------------------------------
            # App mit altem Image starten
            # ----------------------------------------------------
            ...
            rollback_health = check_app_health(
                timeout=60,
                interval=2,
            )
            ...
            if not rollback_health:
                raise RuntimeError(
                    ...
                    "aber die App ist nicht gesund."
                )

            install_status["rollback"] = True
            install_status["status"] = "rolled_back"
            ...
            install_status["status"] = "rollback_failed"

            install_status["error"] = (
                f"Update-Fehler: {exc}; "
                f"Rollback-Fehler: {rollback_exc}"
            )
